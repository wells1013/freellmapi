  ...
  return (
    <section className="rounded-3xl border bg-card p-5">
      <div className="flex items-baseline justify-between mb-3">
        <h2 className="text-sm font-medium">{t('pages.fallback.monthlyTokenBudget')}</h2>
        <span className="text-xs text-muted-foreground tabular-nums">
          <span className="text-foreground font-medium">{formatTokens(remaining)}</span> {t('pages.fallback.remaining')}
          <span className="mx-1.5">·</span>
          {t('pages.fallback.budgetUsage', { pct: remainingPct, total: formatTokens(totalBudget) })}
        </span>
      </div>
      ...
      <div className="flex h-2.5 rounded-full overflow-hidden bg-muted">
        {modelsWithWidth.map((m, i) => (
          <div
            key={i}
            title={t('pages.fallback.tokenUsageRemaining', { displayName: m.displayName, platform: m.platform, tokens: formatTokens(m.remainingTokens) })}
            style={{
              width: `${m.widthPct}%`,
        ...
        {totalUsed > 0 && (
          <div
            title={t('pages.fallback.tokenUsageUsed', { tokens: formatTokens(totalUsed) })}
      ...
      {collapsible && (
        ...
        >
          {expanded ? t('pages.fallback.showLess') : t('pages.fallback.showAll', { count: models.length })}
      ...
      <td className="py-2 pr-3 align-middle">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {row.supportsTools && (
            ...
            >
              {t('pages.fallback.tools')}
            </span>
          )}
          {(row.penalty ?? 0) > 0 && (
            <span className="text-[10px] text-amber-600 dark:text-amber-400">−{row.penalty} {t('pages.fallback.penaltySuffix')}</span>
          )}
          {row.totalRequests !== undefined && row.totalRequests > 0 && (
            <span className="text-[10px] text-muted-foreground/60 tabular-nums">{row.totalRequests} {t('pages.fallback.obsSuffix')}</span>
          )}
        </div>
        <div className="text-[11px] text-muted-foreground/70 tabular-nums mt-0.5">
          {row.monthlyTokenBudget} {t('pages.fallback.tokPerMonth')}
          {row.rpmLimit ? ` · ${row.rpmLimit} ${t('pages.fallback.rpm')}` : ''}
          {row.rpdLimit ? ` · ${row.rpdLimit} ${t('pages.fallback.rpd')}` : ''}
        </div>
      </td>
...
function SortableRow({ row, rank, onToggle }: { row: Row; rank: number; onToggle: (id: number, e: boolean) => void }) {
  ...
  const handle = (
    <button
      ...
      aria-label={t('pages.fallback.dragHandleLabel')}
    >
...
export default function FallbackPage() {
  ...
  const tableHead = (
    <thead>
      <tr className="text-left text-muted-foreground border-b">
        ...
        <th className="py-2 pr-2 w-6 text-center font-medium">{t('pages.fallback.colNum')}</th>
        <th className="py-2 pr-3 font-medium">{t('pages.fallback.colModel')}</th>
        <th className="py-2 pr-3 font-medium">
          <span className="inline-flex items-center gap-1"><span className="size-2 rounded-sm" style={{ background: '#22c55e' }} />{t('pages.fallback.colReliability')}</span>
        </th>
        <th className="py-2 pr-3 font-medium">
          <span className="inline-flex items-center gap-1"><span className="size-2 rounded-sm" style={{ background: '#3b82f6' }} />{t('pages.fallback.colSpeed')}</span>
        </th>
        <th className="py-2 pr-3 font-medium">
          <span className="inline-flex items-center gap-1"><span className="size-2 rounded-sm" style={{ background: '#a855f7' }} />{t('pages.fallback.colIntelligence')}</span>
        </th>
        <th className="py-2 pr-3 font-medium">
          <Tooltip text={t('pages.fallback.guardrailsTooltip')}>
            <span className="underline decoration-dotted underline-offset-2 cursor-help">{t('pages.fallback.colGuardrails')}</span>
          </Tooltip>
        </th>
        <th className="py-2 pr-3 font-medium text-right">
          <Tooltip text={t('pages.fallback.scoreTooltip')}>
            <span className="underline decoration-dotted underline-offset-2 cursor-help">{t('pages.fallback.colScore')}</span>
          </Tooltip>
        </th>
        <th className="py-2 pr-3 font-medium text-right">{t('pages.fallback.colOn')}</th>
      </tr>
    </thead>
  )
  ...
  return (
    <div>
      <PageHeader
        title={t('pages.fallback.header')}
        description={t('pages.fallback.description')}
        divider={false}
        actions={<ModelsTabs />}
      />
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-3xl border bg-card p-5">
          <div className="flex items-baseline justify-between mb-3">
            <h2 className="text-sm font-medium">{t('pages.fallback.routingStrategy')}</h2>
            {routing?.weights && (
              <span className="text-xs text-muted-foreground tabular-nums">
                {t('pages.fallback.weightFormat', {
          ...
          <div className="inline-flex flex-wrap gap-1 rounded-xl border p-1">
            {STRATEGIES.map(s => (
              <Tooltip key={s.key} text={t(`pages.fallback.blurb${s.key.charAt(0).toUpperCase() + s.key.slice(1)}`)}>
                <button
                ...
                >
                  {t(`pages.fallback.strategy${s.key.charAt(0).toUpperCase() + s.key.slice(1)}`)}
                </button>
              </Tooltip>
            ))}
          </div>
          ...
          <p className="mt-2 text-xs text-muted-foreground">
              {isManual
                ? t('pages.fallback.strategyManual')
                : t('pages.fallback.strategyAuto')}
          </p>
        </section>
        ...
        {isLoading ? (
          ...
          <div className="rounded-3xl border border-dashed p-8 text-center">
            <p className="text-sm text-muted-foreground">
              <Trans i18nKey="pages.fallback.noModels" components={[<a href="/keys" className="underline text-foreground" />]} />
            </p>
          </div>
        ) : (
          <>
            ...
            <FloatingBar show={hasChanges}>
              <span className="text-xs text-muted-foreground">{t('pages.fallback.unsavedChanges')}</span>
              <Button variant="outline" size="sm" onClick={() => setLocalEntries(null)}>{t('pages.fallback.discard')}</Button>
              <Button size="sm" onClick={handleSave} disabled={saveMutation.isPending}>
                {saveMutation.isPending ? t('pages.fallback.saving') : t('pages.fallback.saveChanges')}
              </Button>
            </FloatingBar>

            {unconfiguredPlatforms.length > 0 && (
              <p className="text-xs text-muted-foreground">{t('pages.fallback.hiddenPlatforms', { platforms: unconfiguredPlatforms.join(', ') })}</p>
            )}
          </>
        )}
      </div>
